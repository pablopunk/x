...
export const useImageHistory = () => {
        ...
        if (!thumbnailCache[entry.thumbnailId]) {
          try {
            ...
            if (blob) {
              ...
              console.warn(`Thumbnail blob not found for ID: ${entry.thumbnailId}`)
            }
          } catch (error) {
            console.error(`Error fetching thumbnail blob ${entry.thumbnailId}:`, error)
          }
        } else {
          if (idsInLog.has(entry.thumbnailId)) {
            newCacheEntries[entry.thumbnailId] = thumbnailCache[entry.thumbnailId]
          }
        }
      })
        ...
        setThumbnailCache((prevCache) => {
          ...
          historyLog.forEach((logEntry) => {
            if (newCacheEntries[logEntry.thumbnailId]) {
              finalCache[logEntry.thumbnailId] = newCacheEntries[logEntry.thumbnailId]
            } else if (prevCache[logEntry.thumbnailId]) {
              finalCache[logEntry.thumbnailId] = prevCache[logEntry.thumbnailId]
            }
          })
          return finalCache
        })
  ...
  const saveHistoryLogToLocalStorage = useCallback((log: ImageHistoryEntryMetadata[]) => {
    try {
  ...
  const loadImageDataFromMetadata = useCallback(
    async (entryMetadata: ImageHistoryEntryMetadata): Promise<{ image: HTMLImageElement } | null> => {
      ...
      try {
        ...
        if (!imageBlob) {
          console.error("loadImageDataFromMetadata: Image blob not found for ID:", entryMetadata.imageId)
          return null
        }
        ...
        return new Promise((resolve, reject) => {
          ...
          image.onerror = (err) => {
            URL.revokeObjectURL(objectURL)
            console.error("loadImageDataFromMetadata: Failed to load image from blob:", err)
            reject(err)
          }
          image.src = objectURL
        })
      } catch (error) {
        console.error("loadImageDataFromMetadata: Error loading image data:", error)
        return null
      }
    },
    [],
  ...
  const addOrUpdateHistoryEntry = useCallback(
    async (
      ...
      const existingEntryIndex = existingEntryId ? currentLogSnapshot.findIndex((e) => e.id === existingEntryId) : -1
      ...
      if (isAnnotationOnlyUpdate && existingEntryIndex !== -1) {
        ...
        if (JSON.stringify(oldEntry.annotations) === JSON.stringify(annotations)) {
      ...
      try {
        const entryId = existingEntryId || Date.now().toString() + Math.random().toString(36).substring(2, 9)
        let imageId = ""
        let thumbnailId = ""
        ...
        if (imageFile) {
          ...
          const thumbBlob = await createThumbnail(imageBlob, THUMBNAIL_WIDTH, THUMBNAIL_HEIGHT)
          await storeBlob(thumbnailId, thumbBlob)
          const newThumbUrl = URL.createObjectURL(thumbBlob)
          setThumbnailCache((prev) => ({ ...prev, [thumbnailId]: newThumbUrl }))
          ...
          console.error("Cannot update history: No image file and no existing entry ID.")
        ...
        if (entriesToPrune.length > 0) {
          Promise.all(
            entriesToPrune.map((entry) => deleteBlob(entry.imageId).then(() => deleteBlob(entry.thumbnailId))),
          ).then(() => {
            setThumbnailCache((prevCache) => {
              const newCache = { ...prevCache }
              entriesToPrune.forEach((pruned) => {
                if (newCache[pruned.thumbnailId]?.startsWith("blob:")) URL.revokeObjectURL(newCache[pruned.thumbnailId])
  ...
  const loadAndActivateEntryFromMetadata = useCallback(
    async (entryMetadata: ImageHistoryEntryMetadata): Promise<LoadedHistoryEntry | null> => {
      setIsLoading(true) // This is a significant load operation
      try {
        const imageData = await loadImageDataFromMetadata(entryMetadata)
        if (!imageData) {
          console.error("loadAndActivateEntryFromMetadata: Failed to get image data.")
          return null
        }
        ...
        console.error("Failed to load and activate entry from metadata:", error)
        return null
      } finally {
        setIsLoading(false)
      }
  ...
  const loadHistoryEntry = useCallback(
    async (entryId: string): Promise<LoadedHistoryEntry | null> => {
      ...
      if (!entryMetadata) {
        console.warn(`loadHistoryEntry: Metadata not found for entryId ${entryId}.`)
        return null
      }
